package koopa.cobol.parser.dependency;
...
import koopa.cobol.grammar.dependency.test.DependencyCobolProject;
...
public class CobolDependencyParser extends CobolParser {
	...
	private DependencyCobolProject project = null;
	...
	private ParseResults doParse(File file, Parse parse) throws IOException {
		...
		final boolean sawMoreProgramText = grabRemainingProgramText(
				project.getDependencyGrammar(), tail,
	...
	public Parse getParseSetup(File file, Reader reader) throws IOException {
		...
		if (buildTrees)
			parse.to(new KoopaTreeBuilder(project.getDependencyGrammar(), false));
	...
	public void setProject(DependencyCobolProject project) {
		this.project = project;
	}

	private boolean grabRemainingProgramText(CobolDependencyGrammar grammar,
